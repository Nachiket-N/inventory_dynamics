...
const RESULTS_PATH = "inventory_dynamics_project/src/images/NODE/square"
...
function classical_bullwhip_model!(du, u, p, t)
    ...
    square_wave  = sign(sin(2*pi * t / 5))
    du[3]  = kappa*(mu - u[3]) + 8*square_wave * randn()
end
...
res_bfgs  = Optimization.solve(opt_prob2, OptimizationOptimJL.BFGS();
                               maxiters = 500, allow_f_increases = false,
                               callback = cb)
